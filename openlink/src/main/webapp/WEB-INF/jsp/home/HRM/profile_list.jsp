...
function searchProfile(param){
	if(param == 'all'){
		location.href="profileList.do?CT_ID=" + param;
	}else if(param != 'all'){
		location.href="profileList.do?CT_ID=" + param;
	}
}
			...
			<c:if test="${sessionScope.user_id != null && sessionScope.user_id != ''}">
				...
				<div class="panel-body text-center">
					...
					<c:forEach var="cateList" items="${cateList}">
						<button type="button" class="btn btn-primary" onclick="searchProfile('${cateList.ctId}');">${cateList.ctName}</button>
					</c:forEach>
				</div>
				<!-- 검색창 -->
				<div class="panel-body text-center">
					<h4>검색 : 
					<input type="text" id="searchInput" placeholder="이름,소속,상태,등록일 등"></h4>
					...
					<table class="table table-hover">
						...
						<tbody id="myTable">
							<c:if test="${empty HRMList}">
								<td colspan="6" style="text-align:center;">등록된 프로필이 없습니다.</td>
							</c:if>						
							<c:forEach var="HRMList" items="${HRMList}">
								<tr>
									...
									<c:if test="${HRMList.stat == '02'}">
										<td style="text-align:center; color:#FFA500;">휴직</td>
									</c:if>
